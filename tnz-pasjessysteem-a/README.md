...
1. Copy `.env.example` to `.env` and set `SUPABASE_URL` and `SUPABASE_KEY` from Supabase Project Settings > API.
2. Install dependencies: `npm install`
3. Open [supabase/schema.sql](supabase/schema.sql) in the Supabase SQL Editor and run it once. This creates the `profiles` table and synchronizes each registered user's name and e-mail address from `auth.users`.
...
- Keep `.env` local; it is ignored by Git. Never put the `service_role` key in browser code or commit it.
